...
class Voucher < ActiveRecord::Base
  ...
  scope :recent, lambda { |s|
    where('series_id = ?', s.id).order('created_at DESC')
  }
  ...
  def self.incomplete
    unscoped.where('bookkept_by_id is null')
  end
  ...
  def pretty_id
    unless number.nil?
      ...
      "#{activity_year.year}-#{series.letter}##{id}"
    end
  end
  ...
  def set_number!
    if number.nil?
      last_voucher = Voucher.
                     where(activity_year_id: activity_year_id, series_id: series).
                     order('number DESC').
                     first(select: :number)
      if last_voucher
  ...
  def destroy
    if bookkept?
      fail '[Voucher] Tried to delete bookkept voucher!'
    else
...
Attesterat av: #{authorized_by_to_s}
  ...
  def stagnated?
    bookkept? && (minutes_until_stagnation <= 0)
  end
  ...
  def current_voucher_rows
    return voucher_rows if id.nil?
    VoucherRow.find(:all, conditions: { voucher_id: id })
  end

  # Validations

  def check_row_delete(_rows)
    fail '[Voucher] Tried to delete VoucherRows!' if bookkept_validation?
  end

  def check_signature(row)
    fail "[Voucher] Added row lacks signature: #{row.inspect}" if bookkept_validation? && row.signature.nil? && !current_voucher_rows.include?(row)
  end
  ...
  def accounting_date_in_activity_year
    errors[:accounting_date] << I18n.t('activerecord.errors.messages.must_be_in_activity_year') unless activity_year.in_year?(accounting_date)
  end
