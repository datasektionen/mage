class Journal < ActiveRecord::Base
  ...
  def self.log(action, obj, user, api_key=nil)
    action_name = I18n.t("action."<< obj.class.to_s.downcase << "" << action.to_s,:default=>[("action." << action.to_s).to_sym,action])
    model_name = I18n.t('activerecord.models.' << obj.class.to_s.downcase)
    object_data = obj.inspect
    object_data = obj.to_log if obj.respond_to? :to_log
    # suppress warning about changing object_id:
    silence_warnings do 
      create(
        :message=>"#{action_name} #{model_name}:\n#{object_data}",
        :user=>user,
        :api_key=>api_key,
        :object=>obj
      ) or return false
