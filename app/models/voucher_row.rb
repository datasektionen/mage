...
class VoucherRow < ActiveRecord::Base
  ...
  validates_presence_of :arrangement, if: proc { account.has_arrangements? }
  validates_presence_of :account
  validate :no_arrangement, unless: proc { account.has_arrangements? }
  ...
  def destroy
    if voucher.bookkept?
      fail '[VoucherRow] Tried to delete voucher_row from bookkept voucher!'
    else
      super
    end
  end
