module Mage
  class Report
    ...
    def self.full_report_summarized(activity_year, series = nil, organ = nil)
      optional_conditions = build_optional_condition(series, organ)
      return Mage::Reports::Report.generate(
          self.data_from_query("
                          select
                            SUM(voucher_rows.sum) as 'sum',
                            SUM(GREATEST(voucher_rows.sum,0)) as 'debet',
                            SUM(LEAST(voucher_rows.sum,0)) as 'kredit',
                            accounts.ingoing_balance,
                            accounts.name as 'account_name', 
                            account_groups.title as 'account_group_name',
                            account_groups.number as 'account_group_number',
  ...
  def build_optional_condition(series, organ) 
    optional_conditions = ""
